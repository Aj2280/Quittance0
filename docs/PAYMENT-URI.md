...
| 6 | Amount with 8 decimals | — | **refused by the formatter** with a message naming the 7-decimal ceiling; formerly emitted as `amount=1.12345678`, which the SDK refused at payment-build time | closed — was gap 1 |
...
| 9 | XLM with an issuer | `?destination=...&amount=25.0000000` — issuer dropped | pays, but as a **native** payment: the issuer the caller supplied is gone, and nothing says so | yes — gap 5 |
...
2. ~~Refuse amounts with more than seven decimals~~ — **done**: the formatter
...
The four remaining gaps rest on protocol refusals, which is the right authority, but a
